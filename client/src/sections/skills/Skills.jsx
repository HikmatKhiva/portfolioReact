...
import {queryExperience, queryResume, querySkills} from '../../server/queries'
import {useZustandStore} from '../../zustand'
const Skills = () => {
  const {selectedSkill, handleSelect, showResume, toggleShowResume} = useZustandStore()
  const [{data: experience}, {data: resume}, {data: skill}] = useGetQueriyes([
    queryExperience,
    queryResume,
    querySkills,
  ])
  const filterSkill = (id) => skill.filter((skill) => skill._id === id && handleSelect(skill))
  return (
        ...
        <div className="left flex-grow xl:w-1/2  w-full skills-container flex justify-center 2xl:justify-start flex-wrap gap-4">
          ...
          {skill &&
            skill.map((skill, index) => (
              <Skill filterSkill={filterSkill} index={index} key={index} skill={skill} />
            ))}
      ...
      <motion.div className="md:pt-7 py-1 lg:w-full text-center flex gap-4 justify-center">
        {resume && (
          <motion.button
            ...
            onClick={() => download(resume[0].resume, 'resume')}
